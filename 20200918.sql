...
--세가지 컬럼에 데이터 넣을 거여서 컬럼명 기술 안해도 된다.
...
2. 문자 : VARCHAR2(사이즈-byte) / CHAR(X) -> 안쓴다  
...
제약조건 : 데이터에 이상한 값이 들어가지 않도록 강제하는 설정
        ...
        emp테이블에 demptno컬럼의 값이 dept 테이블에 존재하지 않는
        데이터를 입력하는 것을 방지
        emp테이블에 empno(사번) 컬럼의 값이 중복되지 않도록 방지
        
제약조건은 4가지가 존재, 그중에 한가지 제약조건은 2개로 문법을 제공
...
정상적으로 입력될 수 없다.
unique constraint (PC21.SYS_C007083) violated
INSERT INTO dept_test VALUES (90,'ddit','daejeon');
...
CREATE TABLE dept_test (
    ...
    dname VARCHAR2(14),
    loc VARCHAR2(13),
    CONSTRAINT PK_dept_test PRIMARY KEY (deptno ,dname) --어떤 컬럼에 기술할지 ,부서번호와 부서이름이 같으면 제약
    );
...
INSERT INTO emp_test VALUES(9001, 'sally', 10); --에러남 
...
INSERT INTO emp_test VALUES(9000, 'brown', 90);
INSERT INTO emp_test VALUES(9001, 'sally', 10); --제약조건 때문에 데이터가 안들어감
...
만약 dept_test 테이블에서 90번 부서를 삭제하게 된다면??
DELETE dept_test
WHERE deptno = 90;
...
DDL에 의해 자동 커밋, emp_test 테이블에 
CREATE TABLE dept_test (
    deptno NUMBER(2) ,
    dname VARCHAR2(14)
    );
INSERT INTO dept_test VALUES(90, 'brown');
    -- 여기서부터는 새로운 트랜잭션
ROLLBACK;
SELECT *
FROM dept_test;
...
2. 기존에 존재하는 컬럼의 변경(이름, 데이터 타입)
  **데이터 타입의 경우는 이미 데이터가 존재하면 수정이 불가능 하다고 보면 됨.
   동일한 데이터 타입으로 사이즈를 늘리는 경우는 상관없다.
   ==> 설계시 고려를 충분히 하자
...
hp컬럼을 phone으로 컬럼명을 변경 (안에 데이터가 있어도 변경 가능)
