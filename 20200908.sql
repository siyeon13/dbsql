...
TO_CHAR(날짜타입, '변경할 문자열 포멧')
TO_DATE('날짜문자열', '첫번째 인자의 날짜 포멧')
...
SELECT ename,
       -- SUBSTR('20200908' , 1, 4) || '/' || SUBSTR('20200908', 5, 2) || '/' }} SUBSTR('2020)
...
날짜  : 일자 + 시분초
2020년 9월 8일 14시 10분 5초  ==>  '20200908'   ==> 2020년 9월 8일 00ㅅ; 0분 0초

WHERE 
SLEECT TO_DATE(TO_CHAR(SYSDATE, 'YYYYMMDD') )
FROM dual;
...
LAST_DAY(SYSDATE) : SYSDATE(2020/09/08)가 속한 5월의 마지막 날짜 : 2020/09/30
                    월마다 마지막 일자가 다르기 때문에 해당 함수를 통해서 편하게
                    마지막 일자를 구할 수 있다.
해당월의 가장 첫 날짜를 반환하는 함수는 없다 ==> 모든 월의 첫 날짜 1일이다.
FIRST_DAY                    
...
/* SELECT TO_CHAR(SYSDATE, 'YYYYMMDD'),
          TO_CHAR(SYSDATE, 'YYYYMMDD')-1,
           TO_DATE(TO_CHAR(SYSDATE, 'YYYYMMDD')) - TO_DATE((TO_CHAR(SYSDATE, 'YYYYMMDD')-1))
           TO_DATE(T
FROM dual;  */
...
오늘 날짜에서 일자를 구하기 ==> 오늘 날짜 - 구한일자 +1
                    SYSDATE - 8 ==> 2020년 8월 31일 + 1  ==> 2020년 9월 1일


fn3] 주어진 것 : 년월 문자열 ==> 날짜로 변경  ==> 해당월의 마지막 날짜로 변경
...
다만 고급 개발자 외 일반 개발자를 구분하는 차이점이 됨.
...
1600  ==>  1,600(숫자가 아니고 문자)
숫자를 문자로 포멧팅 : DB보다는 국제화(il8n)
                                Internationalization 에서 더 많이 활용
...
NULL 의 특징 : NULL을 포함한 연산의 결과는 항상 NULL
