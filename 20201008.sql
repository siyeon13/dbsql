...
Exceptoin (예외) : pl/sql 블럭이 실행되는 동안 발생하는 에러
                  예외가 발생 했을 대 PL/SQL의 EXCEPTION 절을 기술하여
                  다른 로직을 실행하여 예외 문제를 해결 할 수 있다.
                  
예외 구분
1. 사전 정의 예외(Oracle Predefined Exception)
    오라클에서 미리 정의한 예외로 ORA_XXXXX로 정의
...
EXCEPTION
    ...
    WHEN 예외명3 [OR 예외명4] THEN
        실행할 문장;
    WHEN OTHER THEN
        실행할 문장 (여기서 SQLCODE, SQLERPM 속성을 통해 예외 코드를 확인 할 수 있다.]
END;
/
...
사용자 예외 정의, 사용
PL/SQL 블럭안에서 또다른 PL/SQL 블럭을 정의 하는 것이 가능

NO_EMP : 사번을 통해서 사원을 검색하는데 해당 사번을 갖는 사원이 없을때
    SELECT *
    FROM emp
    WHERE empno = -1;
...
CREATE OR REPLACE FUNCTION getEmpName (p_empno emp.empno%TYPE) RETURN VARCHAR2 IS v_ename emp.ename%TYPE;
BEGIN
    SELECT ename INTO v_ename
    FROM emp
    WHERE empno = p_empno;
...
SELECT empno, empno, deptno ,getDeptName(deptno)
FROM emp;
...
오라클은 10번인자로 getDeptName실행 했을 때 ACCOUNTING이라는 결과 값을 기억(캐싱) 
이후에 동일한 인자로 함수를 실행하면 함수를 실행하지 않고 캐싱된 값을 반환

캐시 - 변화되지 않은 값 그대로 가지고 있음
...
users 테이블의 paa 컬럼이 변경되었을 때 실행할 트리거 생성
...
BEGIN
    --테이블 업데이트가 일어 났을 때, 
    --기존(:OLD.pass) 비밀번호와 업데이트 하려고 하는 비밀번호(:NEW.pass)가 다를 때
...
pass가 아닌 다른값 (alias)만 변경되었기 때문에 트리거가 작동 되지 않는다.
비밀번호 이력이 생기지 않음
SELECT * 
FROM users_history; 
