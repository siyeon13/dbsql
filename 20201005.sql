...
3. expression
    테이블에 존재하지 않지만, 수석, 함수를 이용하여 새로운 컬럼을 만드는 방법
...
CONNECT BY LEVEL <= 31;
    ...
    EX : '202010' ==> 31
         ...
         TO_CHAR(날짜, '원하는 항목')
         TO_CHAR(해당날짜의 마지막 날짜 구하는 함수(TO_DATE('202010', '포멧')), '원하는 항목')
         
         TO_DATE('202010', 'YYYYMM')) : 일자를 설정하지 않았기 때문에 1일자 0시 0분 1초
         --> 마지막 날짜로 변경
         ...
         해당 날짜의 요일에 따라서 새롭게 만든 7개의 컬럼(일~월)ㅇ에 날짜를 위치 시킨다
         날짜+LEVEL1
         해당날짜가 일요일이면 day 표시
         SELECT day, d, iw, DECODE(d, 1, day) d1,  DECODE(d, 2, day) d2,  DECODE(d, 2, day) d2
                            ,  DECODE(d, 3, day) d3,  DECODE(d, 4, day) d4,  DECODE(d, 5, day) d5
                            ,  DECODE(d, 6, day) d6,  DECODE(d, 7, day) d7
         FROM
                (SELECT TO_DATE('202010', 'YYYYMM') + LEVEL-1 day,
                TO_CHAR(TO_DATE('202010', 'YYYYMM') + LEVEL-1, 'D') d,
                TO_CHAR(TO_DATE('202010', 'YYYYMM') + LEVEL-1, 'IW') iw
                FROM dual
         CONNECT BY LEVEL <= TO_CHAR(LAST_DAY(TO_DATE('202010', 'YYYYMM')), 'DD') );
         --iw가 같은 컬럼들을 하나로 묶어서 만들어냐야 한다
         
         
         SELECT   /* DECODE(d, 1, iw+1, iw),   */
                               MIN(DECODE(d, 1, day)) d1,  MIN(DECODE(d, 2, day)) d2,  
                               MIN(DECODE(d, 2, day)) d2
...
DECLARE
    ...
    v_dname VARCHAR2(14);
BEGIN
    SELECT deptno, dname INTO v_deptno, v_dname     --새 변수에 담는다
    FROM dept
    WHERE deptno = 10;
...
CREATE OR REPLACE PROCEDURE printdept (p_deptno IN dept.deptno%TYPE) IS     --dept참조
    v_deptno dept.deptno%TYPE;
    v_dname dept.dname%TYPE;
BEGIN
    SELECT deptno, dname INTO v_deptno, v_dname
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('deptno : ' || p_deptno || ', dname : ' || p_dname || ', loc : ' || p_loc); 
END;
/
